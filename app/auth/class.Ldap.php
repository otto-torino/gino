 ...
 * 
 * ###UTILIZZO
 * Per implementare l'autenticazione attraverso Ldap occorre creare un utente da associare a quello Ldap, che può essere:
...
class Ldap {
	...
	private function checkValidUsername() {
		...
		if(preg_match("#(\\\)+#", $this->_user_name))
		{
			return false;
			/*
			// per uniformare lo username togliendo il nome di dominio evitando che vengano bloccati gli accessi con gli username scritti nella forma "dominio\username"
			$array = explode('\\', $user);
			$last = count($array)-1;
			$user = $array[$last];
			*/
