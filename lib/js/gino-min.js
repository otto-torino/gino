 ...
 */
gino.translations = {
  prepareTrlForm: function(lng_code, el, tbl, field, type, id_value, width, fck_toolbar, url) {
    ...
    if(el != active_trnsl) {
      ...
      gino.ajaxRequest('post', url, data, myTrnsl, {'load':tbl+field, callback: function() { 
        //var url2 = home_file+'?pt[language-replaceTextarea]';	
        //gino.ajaxRequest('post', url2, 'fck_toolbar='+fck_toolbar+'&width='+width+'&field='+field+'&type='+type, null, {'script': true }); 
      }});
    }
  },
  callAction: function(url, type, tbl, field, id_value, ckeditor, lng_code, action) {
    ...
    gino.ajaxRequest(
        'post', 
        url, 
        'type=' + type + '&tbl=' + tbl + '&field=' + field +'&id_value=' + id_value + '&text='+ text +'&lng_code=' + lng_code + '&action=' + action, tbl + field, 
        {
          'callback': function() {
    ...
    if(filled) {
      fieldset.getElements('input,select,textarea').each(function(el) {
        if(el.get('type') == 'radio') {
          if(/checked/.test(console.log(el))) {
            el.setProperty('checked', 'checked');
          }
        }
        el.setProperty('name', rename(field_name, el, i, false));
      })
...
gino.DatePicker = new Class({
  ...
  constructPicker: function() {
    this.picker = new Element('div', { 'class': this.options.pickerClass }).setStyle('z-index', ++gino.maxZindex).inject(document.body);
 ...
 * FORM CALENDAR
 */
gino.printCalendar = function(icon,inputField,days,months) {
  ...
  var e=new gino.DatePicker(inputField,{
...
gino.layerWindow = new Class({
  ...
  request: function() {
    gino.ajaxRequest('post', this.url, '', this.body, {'script':true, 'load':this.body, 'callback':this.locateContainer.bind(this)});	 
  },
  setFocus: function() {
