...
Action:
	...
	public function actionUpdateOrder() {
		...
		foreach($items as $item) {
			$sort = new sort(array('id'=>$item, 'instance'=>$this->_instance, 'table'=>calendarEvent::$_tbl_selection, 'field_id'=>'reference'));
			$sort->priority = $i;
			$sort->updateDbData();
			$i++;
		}
...
class sort {
	...
	public function jsLib($path_action){
		...
		$GINO .= "var prioritySortables = new Sortables($('{$this->_p['ul_id']}'), {
					...
					onComplete: function() {
						...
						ajaxRequest('post', '$path_action', 'order='+order+'&tbl={$this->_p['table']}', null, {'callback':message});
       					}
	...
	public function newPriority() {
		...
		if($where AND $this->_p['aggregator'])
			$where .= " AND aggregator='{$this->_p['aggregator']}'";
