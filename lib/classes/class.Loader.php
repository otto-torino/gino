...
namespace Gino {
    ...
    class Loader {
         ...
         *              Per importare classi core, l'argomento $app deve avere3 la forma [class | class/subdir] dove subdir è la subdir di lib/classes in cui si trova la classe da importare,
         *              mentre le classi da importare, argomento $classes, devono essere complete di namespace.
         *              Per importare classi di tipo Model invece l'argomento $app deve essere il nome del Controller (ed anche della directory della app), mentre le classi nell'argomento 
            ...
            // models
                ...
                foreach($classes as $class_name) {
                    ...
                    if(!class_exists(get_model_app_name_class_ns($app, $class_name), false)) {
                        trigger_error(sprintf(_("Unable to load the model class: %s"), $class_name), E_USER_WARNING);
                    }
                }
            }
